...
```bash
docker run -d --restart=unless-stopped --name=pairdrop -p 127.0.0.1:8000:8000 lscr.io/linuxserver/pairdrop
```
...
```bash
docker run -d --restart=unless-stopped --name=pairdrop -p 127.0.0.1:8000:8000 ghcr.io/schlagmichdoch/pairdrop
```
...
```bash
docker run -d --restart=unless-stopped --name=pairdrop -p 127.0.0.1:8000:8000 -it pairdrop
```
...
>
> - 8000 -> `-p 127.0.0.1:8000:8000`
> - 8080 -> `-p 127.0.0.1:8080:8000`

#### Set Environment Variables via Docker

Environment Variables are set directly in the `docker run` command: \
e.g. `docker run -p 127.0.0.1:8000:8000 -it pairdrop -e DEBUG_MODE="true"`
...
> By default, the node server listens on port 8000.
...
```bash
PORT=8000 RTC_CONFIG="rtc_config.json" npm start
```

On Windows

```bash
$env:PORT=8000 RTC_CONFIG="rtc_config.json"; npm start
```
...
> Examples of valid values: `pairdrop.net`, `pairdrop.your-domain.com:8000`, `your-domain.com/pairdrop`
...
<VirtualHost *:443>
	ProxyPass / https://127.0.0.1:8000/ upgrade=websocket
</VirtualHost>
```
...
```apacheconf
<VirtualHost *:80>
	Redirect permanent / https://127.0.0.1:8000/
</VirtualHost>
<VirtualHost *:443>
	ProxyPass / http://127.0.0.1:8000/ upgrade=websocket
</VirtualHost>
...
- The feature `Insecure origins treated as secure` must be enabled and the list must include your PairDrop test instance. E.g.: `http://127.0.0.1:8000,https://127.0.0.1:8443`
